...
fn main() {
    ...
    let child1 = thread::spawn(move || {
        s1.send("Hello from child1").unwrap();
        drop(s1);
        // r1.recv().unwrap();
        // for msg in r1 {
        //     println!("child1: {}", msg);
    ...
    let child2 = thread::spawn(move || {
        s2.send("Hello from child2").unwrap();
        drop(s2);
        // Messages can only be received by one thread.
        // I am not sure why when I receive the message inside a thread, i lost one message from Main Thread.
        // This suppose to have 5 messages from main thread, but this only print 4 main thread messages.
        // The unwrap call will crash if there are multiple receiver.
        r2.recv().unwrap();
        for msg in r2 {
            println!("child2: {}", msg);
        }
    ...
    for i in 0..5 {
        s.send("From Main Thread").unwrap();
    }

    // You must drop s, otherwise it will block.
    drop(s);
